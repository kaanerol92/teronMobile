...
class LoginScreenView extends State<LoginViewCommand> {
  ...
  void giris(BuildContext context) {
    ...
    futureGiris(context).then((value) {
      ...
      if (value == null) {
        ...
        menuMap.putIfAbsent(
            'Stok İşlemleri', () => LoadingScreenViewCommand("keke"));
        Navigator.push(context, MaterialPageRoute(builder: (context) {
  ...
  Future<KullaniciSessionModel> futureGiris(BuildContext context) async {
    ...
    var url =
        "http://192.168.2.84:8080/ERPService/login/kullanici?personel_kodu=$personel&sifre=$sifreLink&donem_kodu=$selectedDonem&sirket_kodu=$selectedSirket";
    var response;
    try {
