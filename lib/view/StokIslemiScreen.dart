...
class StokIslemiScreen extends BaseFisScreen {
  ...
  showIsOkDialog(BuildContext context) {
    String fisStr;
    if (fisTipi == 1) {
      fisStr = TextRepository.getText(TextRepository.MAL_ALIM);
    } else if (fisTipi == 10) {
      fisStr = TextRepository.getText(TextRepository.URETIM);
    } else if (fisTipi == 30) {
      fisStr = TextRepository.getText(TextRepository.TOPTAN_SATIS);
    } else if (fisTipi == 31) {
      fisStr = TextRepository.getText(TextRepository.PERAKENDE_SATIS);
    }
      ...
      onPressed: () {
        ...
        model.insert(satirlarRowModel).then((_) {
          ...
          AlertDialog alert = AlertDialog(
            ...
            content: Text("$fisStr fişiniz $fisNo no ile oluşmuştur."),
            actions: [
              FlatButton(
                  onPressed: () {
                    Navigator.pop(context);
                    if (fisTipi == 1) {
                      Navigator.popAndPushNamed(context, '/malalim');
                    } else if (fisTipi == 30) {
    ...
    AlertDialog alert = AlertDialog(
      title: Text("Uyarı!"),
      content: Text("$fisStr fişi oluşturmak istediğinizden emin misiniz?"),
      actions: [
        cancelButton,
        continueButton,
      ],
